    ...
    private var trip: Trip? {
        didSet {
            ...
            if user.accountType == .driver {
                ...
                print("DEBUG: Show ride action view for acepted trip..")
            }
        }
    }
...
private extension HomeController {
    func searchBy(natureLanguageQuery: String, completion: @escaping([MKPlacemark]) -> Void) {
        var results = [MKPlacemark]()
        
        let request = MKLocalSearch.Request()
        request.region = mapView.region
        request.naturalLanguageQuery = natureLanguageQuery
...
extension HomeController: LocationInputViewDelegate {
    func executeSearch(query: String) {
        searchBy(natureLanguageQuery: query) { (results) in
